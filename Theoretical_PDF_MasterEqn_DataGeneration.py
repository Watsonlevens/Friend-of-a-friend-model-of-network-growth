...
#Master equation approximation 
k=np.arange(0,9158,1) # Degrees  95743
P=np.zeros(len(k))
...
for kc in k[3:]: # Proportion of nodes with degree k>=3
    P[kc]= (p+q*(kc-1)/EX)/(1+p+q*kc/EX)*P[kc-1]# Proportion of nodes with degree k>=3
...
#Plot master equation
ax.loglog(k,P, alpha=0.5,label = 'Master equation approximation', color='red')
